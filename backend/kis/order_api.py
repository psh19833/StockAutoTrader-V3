...
def submit_cash_order(
    ...
    if strict_validation and not risk_decision_approved:
        return OrderSubmitResult(False, message="Risk decision not approved", error_type="RISK_NOT_APPROVED")
    if strict_validation and not account_no:
        return OrderSubmitResult(False, message="Missing account_no", error_type="ACCOUNT_REQUIRED")
    if strict_validation and not account_product_code:
        return OrderSubmitResult(False, message="Missing account product code", error_type="ACCOUNT_PRODUCT_REQUIRED")
    if strict_validation and not correlation_id:
